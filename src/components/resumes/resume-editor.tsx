      ...
      <div className="editor-layout">
        <section className="editor-form-panel">
          ...
          <form className="editor-form" onSubmit={handleSubmit(saveResume)}>
            ...
            {enabledSections.includes("projects") ? <div className="editor-section-block"><div className="flex items-center justify-between"><BlockLabel>Projects</BlockLabel></div><RepeatList items={projects.fields} onAdd={() => projects.append({ ...emptyProject, id: `project-${Date.now()}` })} addLabel="Add project" emptyHint="Add a project to show it on the resume.">{(index) => <EntryShell index={index} onRemove={() => projects.remove(index)} removable title="Project"><EditorField label="Name" name={`projects.${index}.name`} register={register} /><EditorField label="GitHub link" name={`projects.${index}.url`} register={register} placeholder="github.com/your-name/project" /><EditorField label="Live link" name={`projects.${index}.liveUrl`} register={register} placeholder="https://project.dev" /><EditorField label="Description" name={`projects.${index}.description`} register={register} textarea /><TagListField label="Technologies, comma separated" value={watch(`projects.${index}.technologies`) ?? []} placeholder="Figma, React, Notion" onCommit={(technologies) => setValue(`projects.${index}.technologies`, technologies, { shouldDirty: true })} /></EntryShell>}</RepeatList></div> : null}
